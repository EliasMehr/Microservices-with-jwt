 ...
 * Service application about changes to the users and permission table respectively, which allows this application
 ...
 * permission is created for that user. After that, the user may log in with the email and password they registered
 ...
 * the user not only exists, but is enabled and has permission. If login is successful, the user's role and a
 ...
 * user id. The user and permission are retrieved from the database via the user id from the token. The user must be
 * a valid user that is enabled and have permission which has not been revoked. If the security check passes, then
 ...
 *
 * Swagger API documentation for all services that use a controller can be viewed at "http://localhost:8080/swagger-ui/"
 *
